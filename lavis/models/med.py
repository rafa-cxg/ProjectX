...
from lavis.models.vit import PretrainVisionTransformerDecoder
...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = self.position_ids[#每个tocken都有p_id
...
class UnifiedBertEmbeddings(nn.Module):
    """Construct the embeddings from word and position embeddings."""
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        self,
        hidden_states,
        attention_mask=None,#0 or -10000
        head_mask=None,
        ...
        attention_probs = nn.Softmax(dim=-1)(attention_scores)#在这里softmax,padding部分的score是无限小的负数
        ...
        outputs = outputs + (past_key_value,)# triple:2
        return outputs
...
class BertAttention(nn.Module):
    '''
    composed of BertSelfAttention+SelfOutputs
    '''
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        outputs = self_attention_outputs[1:-1]# todo 似乎没用？
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        try:
            # ALBEF
            fusion_layer = self.config.fusion_layer #6
        except AttributeError:
            # BLIP
        ...
        for i in range(start_layer, output_layer):
            layer_module = self.layer[i]#BertLayer, including: attention, intermediate, output
            ...
            if self.gradient_checkpointing and self.training: #default: false
...
class BertPredictionHeadTransform(nn.Module):
    '''
    在BertLMPredictionHead调用,只是一个MLP Module
    '''
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
            self.transform_act_fn = ACT2FN[config.hidden_act]#choose a activate function
        else:
...
class BertOnlyMLMHead(nn.Module):
    '''
    wrap the BertLMPredictionHead as well as BertPredictionHeadTransform
    '''
    def __init__(self, config):
        super().__init__()
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            # cf https://github.com/pytorch/pytorch/pull/5617信
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = BertEmbeddings(config)# word and position embedding
    ...
    def forward(
        self,
        input_ids=None,# 0 is [cls],1 is padding, 2 is [sep]
        attention_mask=None,
        token_type_ids=None,
        position_ids=None,
        head_mask=None,
        ...
        # We can provide a self-attention mask of dimensions [batch_size, from_seq_length, to_seq_length]
        # ourselves in which case we just need to make it broadcastable to all heads.
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(#if is decoder,则是下三角形为0，上三角为-10000
        ...
        if encoder_embeds is None:
            embedding_output = self.embeddings( #achieve input_embed+pos+tocken_type_embed
                input_ids=input_ids,
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        prediction_scores = self.text_cls(sequence_output)
        ...
        if labels is not None:
            loss_fct = CrossEntropyLoss()  # -100 index = not masked tocken
            masked_lm_loss = loss_fct(
                prediction_scores.view(-1, self.config.vocab_size), labels.view(-1) #todo 待定 #[:,2,:] represent relation tocken
            )
        ...
        return MaskedLMOutput(
            ...
            attentions=outputs.attentions,# default:none
        )
...
class UnifiedBertForMaskedLM(BertPreTrainedModel):
    ...
    def from_config(cls, cfg, from_pretrained=False):
        config_unified_encoder = BertConfig.from_json_file(
            get_abs_path(cfg["unified_med_config_path"])
        )

        if from_pretrained:
            logger.info(" loading unified encoder weights from '{}'".format("bert-base-uncased"))
            return cls.from_pretrained(
                "bert-base-uncased", config=config_unified_encoder,model_cfg=cfg
            )

        else:
            return cls(config=config_unified_encoder)
    ...
    def __init__(self, config,model_cfg):
        ...
        self.encoder_to_decoder = nn.Linear(768, 512, bias=False)
        self.text_cls = BertOnlyMLMHead(self.bertconfig)
        self.mask_token = nn.Parameter(torch.zeros(1, 1, self.bertconfig.hidden_size))
    ...
    def get_output_embeddings(self):
        return self.text_cls.predictions.decoder

    def set_output_embeddings(self, new_embeddings):
        self.text_cls.predictions.decoder = new_embeddings
    ...
    def forward(
        ...
        x = self.patch_embed(input_images)
        if patch_mask != None:
            if patch_mask is not None:
                # replace the masked visual tokens by mask_token
                flatten_patch_mask = patch_mask.view(B,-1).unsqueeze(-1).type_as(self.mask_token)
                x = x * (1 - flatten_patch_mask) + self.mask_token * flatten_patch_mask



        cls_tokens = self.cls_token.expand(
            B, -1, -1
        )  # stole cls_tokens impl from Phil Wang, thanks #todo 为什么可以重复同样的cls在整个batch size上？
        ...
        unified_attention_mask = torch.cat((image_atts, text_attention_mask), 1)

        if token_type_ids == None:
            token_type_ids = torch.zeros((B, self.num_patches+1 + self.model_cfg.max_txt_len), dtype=torch.long).to(
                # (bs,1+um_patch)
                self.device
            )
            token_type_ids[:,self.num_patches+1:] = 1
        embedding_output = self.embeddings(  # achieve input_embed+pos+tocken_type_embed

            position_ids=position_ids,
            token_type_ids=token_type_ids,
            inputs_embeds=unified_embeddings,
        )
        outputs = self.bert(
        ...
        sequence_output = outputs[0]
        prediction_scores = self.text_cls(sequence_output[:,self.num_patches+1:,:])
        ...
        if labels is not None and patch_mask != None:
            ...
            loss_fct = nn.MSELoss()
            masked_loss = loss_fct(
                prediction_scores, selected_labels
            )
        ...
        if not return_dict:
            output = (prediction_scores,) + outputs[2:]
            return (
                ((masked_loss,) + output) if masked_loss is not None else output
            )
...
class BertLMHeadModel(BertPreTrainedModel):#做decoder
    ...
    def from_config(cls, cfg, from_pretrained=False):
        ...
        med_config_path = get_abs_path(cfg.get("med_config_path"))  # config of bert
    ...
    def __init__(self, config):
        ...
        self.init_weights()
        self.cls.predictions.decoder = nn.Linear(config.encoder_width, config.decoder_vocab_size)
...
class XBertEncoder(BertModel, BaseEncoder):
    ...
    def forward_automask(self, tokenized_text, visual_embeds, **kwargs):#必然是个multimodel encoder
        image_atts = torch.ones(visual_embeds.size()[:-1], dtype=torch.long).to(
            self.device
        )
