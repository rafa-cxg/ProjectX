...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1):
        ...
        x = x + self.pos_embed[:, : x.size(1), :]#todo 为什么这么取？
        x = self.pos_drop(x)

        for i, blk in enumerate(self.blocks):
            x = blk(x, register_blk == i)
        x = self.norm(x) #(bs,num_patches + 1,hidden size)
...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, num_patches=196,
                 ):
        super().__init__()
    ...
    def forward(self, x, label, return_token_num=None):
        for blk in self.blocks:
            x = blk(x)

        if return_token_num != None:
            # B = return_token_num.shape[0]
            # num_patches = return_token_num.shape[1]

            # return_token_num=return_token_num.view(-1)# minibatch每个样本被mask的数目不同，所以把mini batch中的所有样本合在一起
            index = torch.where(return_token_num)
            flat_mask = torch.stack((index[0],index[1]),0)
            # x = x.view(-1,x.shape[-1])
            masked_feat = []
            selected_label =[]#只保留被mask区域的label
            for mini_mask in zip(flat_mask[0],flat_mask[1]):

                masked_feat.append(self.head(self.norm(x[mini_mask]))) # only return the mask tokens predict pixels
                selected_label.append(label[mini_mask])
            masked_feat = torch.stack(masked_feat,0)
            selected_label = torch.stack(selected_label, 0)
        else:
            x = self.head(self.norm(x)) # [N, 3*16^2]

        return masked_feat,selected_label